...
const MSG_PREFIX = 'jlc-eda-design-companion.';
...
function dbg(label: string, obj?: any): void {
	console.log('[JlcEdaCompanion][ext] ' + label, obj !== undefined ? obj : '');
}
...
class BridgeClient {
	...
	async postJson(path: string, payload: any): Promise<any> {
		...
		try {
			...
			if (allPolylines) {
				...
				for (const poly of allPolylines) {
					...
					} catch(e2) { console.log('[JlcEdaCompanion] serialize err:', e2); }
...
async function applyResults(result: any, netsToRoute: string[], unitsMm: boolean = false, replaceExisting: boolean = false): Promise<{tracksCreated: number; viasCreated: number; tracksRemoved: number}> {
	const netsSet = new Set(netsToRoute);

	// Existing copper is only deleted after an explicit user opt-in from the UI.
	let tracksRemoved = 0;
	if (replaceExisting) {
	try {
...
onIframeMessage('start-routing', async (config: any) => {
	...
	isRoutingInProgress = true; _G.__jlcEdaCompanionRouting = true;
	...
	try {
		...
		currentJobId = jobId; _G.__jlcEdaCompanionJobId = jobId;
	} catch (e: any) {
	...
	if (finalStatus !== 'completed') {
		...
		currentJobId = null; _G.__jlcEdaCompanionJobId = null;
		return;
	}
	...
	sendToIframe('routing-complete', {
		...
		isRoutingInProgress = false; _G.__jlcEdaCompanionRouting = false;
	}
});
...
async function collectPreflight(): Promise<any> {
	const client = new BridgeClient();
	const serverOk = await client.checkServer();
	if (!serverOk) {
...
async function openPreflightDialog(): Promise<void> {
	try {
...
export async function runBoardPreflight(): Promise<void> {
	try {
