...
# ---------------------------------------------------------------------------
# Stimulus / load overrides (opt-in — default is a no-op so the rendered
# netlist stays byte-identical to the historical output)
# ---------------------------------------------------------------------------

# Module-level options set by the orchestrator (simulate_subcircuits) before
# rendering. None means "keep the historical zero-source-impedance / fixed
# load behavior". Kept as module state so all generators get it for free
# without changing their signatures.
_SOURCE_IMPEDANCE = None  # ohms (float) to put in series with each AC source
_LOAD_OHMS = None         # ohms (float) to override existing fixed load resistors

# Matches a SPICE voltage source carrying an AC stimulus, e.g.
#   VAC ac_in 0 DC 0 AC 1
#   V_VCC vcc 0 DC 5         (no AC token — NOT matched)
_AC_VSRC_RE = re.compile(
    r'^(?P<name>V\w+)\s+(?P<pos>\S+)\s+(?P<neg>\S+)\s+(?P<rest>.*\bAC\b.*)$'
)

# Matches a fixed load resistor line whose name starts with RLOAD/Rload, e.g.
#   RLOAD {drain} 0 1k
#   Rload mid 0 100
_LOAD_RES_RE = re.compile(
    r'^(?P<name>R(?:LOAD|load)\w*)\s+(?P<n1>\S+)\s+(?P<n2>\S+)\s+(?P<val>\S+)\s*$'
)


def set_sim_options(source_impedance=None, load=None):
    """Configure stimulus/load overrides for subsequent testbench renders.

    Args:
        source_impedance: Ohms to insert in series with each AC voltage
                          source (None = ideal zero-impedance source, the
                          historical behavior).
        load: Ohms to use for fixed load resistors, overriding the
              hardcoded values (None = keep the per-template default).
    """
    global _SOURCE_IMPEDANCE, _LOAD_OHMS
    _SOURCE_IMPEDANCE = source_impedance
    _LOAD_OHMS = load


def _apply_sim_options(circuit):
    """Rewrite a netlist string to honor source-impedance / load overrides.

    Returns (new_circuit, applied_extra_vars). When no options are set the
    circuit is returned unchanged and applied_extra_vars is empty, so the
    output is byte-identical to the historical netlist.

    - source_impedance: each AC voltage source `Vx p n ... AC ...` is split
      so the source drives an internal node `p__src` and a series resistor
      `Rsrc_x p__src p <ohms>` feeds the original node — i.e. a real source
      internal resistance.
    - load: any fixed load resistor line (name starts with RLOAD/Rload) has
      its value replaced with <ohms>.
    """
    if _SOURCE_IMPEDANCE is None and _LOAD_OHMS is None:
        return circuit, {}
    ...
    for line in circuit.split("\n"):
        ...
        if _SOURCE_IMPEDANCE is not None:
            m = _AC_VSRC_RE.match(line.strip())
            if m and line.strip() == line:  # only real netlist lines, not comments
                name = m.group("name")
                pos = m.group("pos")
                neg = m.group("neg")
                rest = m.group("rest")
                src_node = f"{pos}__src"
                r_ohms = _format_eng(_SOURCE_IMPEDANCE)
                out_lines.append(f"{name} {src_node} {neg} {rest}")
                out_lines.append(f"Rsrc_{name} {src_node} {pos} {r_ohms}")
                applied["source_impedance"] = str(_SOURCE_IMPEDANCE)
                continue

        if _LOAD_OHMS is not None:
            m = _LOAD_RES_RE.match(line)
            if m:
                r_ohms = _format_eng(_LOAD_OHMS)
                new_line = f"{m.group('name')} {m.group('n1')} {m.group('n2')} {r_ohms}"
                applied["load_ohms"] = str(_LOAD_OHMS)
...
class SpiceTestbench:
    ...
    def render(self, backend, output_file):
        ...
        # Apply stimulus/load overrides if configured (no-op by default,
        # so the rendered netlist is byte-identical when unset).
        circuit, applied = _apply_sim_options(self.circuit)
        extra_vars = self.extra_vars
        if applied:
            extra_vars = dict(extra_vars)
            extra_vars.update(applied)

        meas_block = backend.format_measurement_block(
            self.analyses, self.measurements, output_file, extra_vars
        )
        return f"{circuit}\n{meas_block}\n\n.end\n"
