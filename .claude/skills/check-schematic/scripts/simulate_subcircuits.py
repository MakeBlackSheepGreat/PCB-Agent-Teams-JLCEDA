...
def simulate_subcircuits(analysis_json, workdir=None, timeout=5, types=None,
                         ...
                         mc_seed=42, source_impedance=None, load=None,
                         corner=True):
    ...
    Args:
        ...
        source_impedance: Ohms in series with each AC stimulus source
                          (None = ideal zero-impedance source, default)
        load: Ohms to override hardcoded load resistors (None = keep per-template
              default)
        corner: When True (and monte_carlo_n>0), also run a deterministic
                worst-case corner analysis alongside Monte Carlo

    Returns:
        Report dict with simulation results
    """
    # Configure opt-in stimulus/load overrides (no-op when both None, so the
    # generated netlist stays byte-identical to the historical output).
    from spice_templates import set_sim_options
    set_sim_options(source_impedance=source_impedance, load=load)
        ...
        for i, det in enumerate(detections):
            ...
            if monte_carlo_n > 0 and result.get("status") != "skip":
                ...
                if components:
                    ...
                    # Deterministic worst-case corner analysis — hits the
                    # tolerance-band extremes directly, so its spread is the
                    # real worst case (vs. MC's statistical estimate).
                    if corner:
                        from spice_tolerance import (
                            corner_factors, apply_corner,
                            aggregate_corner_results,
                        )
                        factors_sets = corner_factors(components)
                        corner_results_list = []
                        cn_cir = os.path.join(workdir, f"{label}_corner.cir")
                        cn_out = os.path.join(workdir, f"{label}_corner.out")
                        cn_log = os.path.join(workdir, f"{label}_corner.log")
                        for factors in factors_sets:
                            cornered_det = apply_corner(
                                det, components, factors, det_type=det_type)
                            cn_result, _cn_elapsed = _run_single_sim(
                                cornered_det, det_type, generator, evaluator,
                                cn_cir, cn_out, cn_log)
                            if cn_result is not None:
                                corner_results_list.append(cn_result)
                        result["corner_analysis"] = aggregate_corner_results(
                            result, corner_results_list, components,
                            _singular_type(det_type), det_type=det_type,
                        )
                        for f in (cn_cir, cn_out, cn_log):
                            if os.path.exists(f):
                                os.remove(f)
    ...
    # Record the stimulus/load environment actually used (omit when default
    # so the report shape is unchanged for the common case).
    if source_impedance is not None or load is not None:
        report["stimulus"] = {
            "source_impedance_ohm": source_impedance,
            "load_ohm": load,
        }

    # Reset module-level overrides so they don't leak into later calls.
    set_sim_options(source_impedance=None, load=None)
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="OHM",
        help="Series resistance (ohms) for each AC stimulus source. Default: "
             "ideal zero-impedance source (matches historical behavior).",
    )
    parser.add_argument(
        "--load",
        type=float,
        default=None,
        metavar="OHM",
        help="Override the hardcoded load resistor value (ohms) in testbenches "
             "that use a fixed load. Default: keep each template's value.",
    )
    parser.add_argument(
        "--no-corner",
        dest="corner",
        action="store_false",
        help="Disable the deterministic worst-case corner analysis that is "
             "otherwise appended to Monte Carlo runs (--monte-carlo).",
    )
    parser.set_defaults(corner=True)
    ...
    if args.analysis_dir:
        # analysis_cache lives in this script's own dir — already on sys.path
    ...
    report = simulate_subcircuits(
        ...
        source_impedance=args.source_impedance,
        load=args.load,
        corner=args.corner,
    )
