...
def build_net_map(components: list[dict], wires: list[dict], labels: list[dict],
    ...
    # Add component pins. PWR_FLAG is an ERC source marker — its lib pin is
    # `power_out`. We MUST keep it in net.pins so audit_rail_sources can detect
    # the rail as sourced (either via the `pin_type==power_out` branch or the
    # `comp.startswith("#FLG")` branch in _has_direct_source). Skipping it here
    # silently breaks the entire RS-001 → PWR_FLAG cleanup path.
    for comp in components:
...
def audit_datasheet_coverage(components: list[dict],
    ...
    import os as _os
    pd = project_dir or "."
    # Walk up to 3 levels to support common nested layouts:
    #   <project>/datasheets/                     ← project root
    #   <project>/kicad/datasheets/               ← kicad subdir
    #   <project>/kicad/<gen>/datasheets/         ← circuit-synth generated dir (2 deep)
    candidates = [
        ...
        _os.path.join(_os.path.dirname(_os.path.dirname(pd)), "datasheets"),
    ]
    ds_dir_found = next(
