...
def _classify_model_provenance(model_source):
    """Map a model source note to a structured provenance/confidence record.

    model_source comes from spice_model_cache.get_model_for_part() and looks
    like "cache:LM358", "api:lcsc", "datasheet:LM358", "lookup:LM358", or
    "ideal"/"0". The confidence reflects how much to trust the model's
    parameters — datasheet/api specs are the source of truth, the lookup
    table is curated-but-static, ideal is a topology-only fallback.

    Returns:
        Dict {"source": <category>, "raw": <model_source>, "confidence": <level>}
    """
    raw = model_source or "ideal"
    src = raw.split(":", 1)[0].lower() if isinstance(raw, str) else "ideal"

    if src in ("", "ideal", "0"):
        source, confidence = "default", "low"
    elif src == "datasheet":
        source, confidence = "datasheet", "high"
    elif src == "api":
        source, confidence = "api", "high"
    elif src == "cache":
        # Cached models were resolved earlier from one of the above tiers;
        # the tier isn't recorded in the note, so treat as medium.
        source, confidence = "cache", "medium"
    elif src == "lookup":
        source, confidence = "lookup", "medium"
    else:
        source, confidence = src, "low"

    return {"source": source, "raw": raw, "confidence": confidence}
...
def evaluate_opamp_circuit(det, sim_results):
    ...
    result = {
        ...
        "model_provenance": _classify_model_provenance(model_source),
    }
