...
# Honesty caveat: PS-001 only inspects enable-pin connectivity and PG presence.
# It does NOT verify actual power-up/power-down timing (rise-time ordering,
# t_delay between rails) — that needs a transient simulation or bench review.
_PS_001_CAVEAT = (
    ' NOTE: this is a connectivity check only — it does NOT verify actual '
    'power-up/down timing (rail ordering, delays); validate sequencing with a '
    'transient simulation or bench measurement.'
)

# Honesty caveat: FS-001 only checks the feedback divider impedance range and
# presence of a compensation cap. It does NOT compute loop gain, phase margin,
# or crossover frequency — closed-loop stability is unverified here.
_FS_001_CAVEAT = (
    ' NOTE: this is a connectivity/impedance-range check only — it does NOT '
    'verify closed-loop stability (loop gain, phase margin, crossover); run an '
    'AC loop simulation or design review to confirm stability.'
)
        ...
        if cycle:
            ...
            if cycle_key not in reported_cycles:
                ...
                findings.append(make_finding(
                    ...
                    description=f'Power regulator enable chain forms a cycle: {" -> ".join(cycle)}. No regulator in the cycle can start.' + _PS_001_CAVEAT,
                    severity='error', confidence='deterministic', evidence_source='topology',
        ...
        if not pg_net:
            if ds_feat is None and mpn:
                findings.append(make_finding(
                    ...
                    description=f'Regulator {ref} ({mpn}) supplies {output_rail} to {", ".join(sensitive_loads[:3])} but PG status is unknown. Run sync_datasheets to verify.' + _PS_001_CAVEAT,
                    severity='info', confidence='heuristic', evidence_source='topology',
            ...
            findings.append(make_finding(
                ...
                description=f'Regulator {ref} supplies {output_rail} to sequencing-sensitive device(s) ({", ".join(sensitive_loads[:3])}) but has no PG output connected.' + _PS_001_CAVEAT,
                severity='warning', confidence='heuristic', evidence_source='topology',
                components=[ref] + sensitive_loads[:3], nets=[output_rail],
    ...
    for reg in power_regulators:
        ...
        if parallel_impedance < _FB_IMPEDANCE_MIN:
            findings.append(make_finding(
                ...
                description=f'Feedback divider for {ref} has parallel impedance of {parallel_impedance:.0f} ohms. Low impedance wastes quiescent current.' + _FS_001_CAVEAT,
                severity='info', confidence='heuristic', evidence_source='topology',
        ...
        if parallel_impedance > _FB_IMPEDANCE_MAX:
            findings.append(make_finding(
                ...
                description=f'Feedback divider for {ref} has parallel impedance of {parallel_impedance/1000:.0f}k ohms. High impedance is noise-susceptible.' + _FS_001_CAVEAT,
                severity='warning', confidence='heuristic', evidence_source='topology',
                components=[ref, fb_div['r_top'].get('ref', ''), fb_div['r_bottom'].get('ref', '')],
            ...
            if check_net:
                ...
                if not caps_on_fb:
                    findings.append(make_finding(
                        ...
                        description=f'Regulator {ref} ({comp.get("value", "")}) typically requires external compensation on {check_net}.' + _FS_001_CAVEAT,
