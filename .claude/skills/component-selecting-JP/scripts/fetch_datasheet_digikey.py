...
def search_digikey(mpn: str) -> dict | None:
    ...
    # Search for the part. Pass locale headers so API returns JP-localized
    # data (JPY price + JP-active status) instead of US/USD defaults.
    # Headers can be overridden via env (DIGIKEY_LOCALE_SITE / _CURRENCY).
    locale_site = os.environ.get("DIGIKEY_LOCALE_SITE", "JP")
    locale_currency = os.environ.get("DIGIKEY_LOCALE_CURRENCY", "JPY")
    locale_lang = os.environ.get("DIGIKEY_LOCALE_LANGUAGE", "en")
    try:
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "X-DIGIKEY-Locale-Language": locale_lang,
            },
        )
        with urllib.request.urlopen(req, timeout=15) as resp:
